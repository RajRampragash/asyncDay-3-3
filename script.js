            ...
            try {
                ...
                if (i["name"]["nativeName"]) {
                    ...
                    for (n of arr) {
                        ...
                        //console.log("ref", i.name.nativeName[n]);
                ...
                cardtext.innerHTML = `<p> <label>Region :</label> ${i.region}, <br> <label>Population :</label> ${i.population}, <br>        <label>Native Name :</label> ${nn}</p> 
                <p class="weather"></p>`
...
async function weatherdataAPI(e) {
    ...
    w.innerText = `Today's weather in ${c2.name} : ${c2.weather[0].description}. `;
}